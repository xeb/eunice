...
- Gemini 3.x models (`gemini-3*`, e.g. `gemini-3.8-flash` (the default), `gemini-3.1-pro-preview`): Use native Gemini API (`/v1beta/models/{model}:generateContent`) — required because Gemini 3 thought signatures must be round-tripped on function calls
